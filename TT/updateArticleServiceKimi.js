  ...
  ui.layout(
    <frame>
      <vertical h="auto" marginTop="20">
        <linear marginTop="30">
          <button id="them_to_article_btn" bg="#8acfaa" text="生成Kim文章" marginLeft="35" h="38" />
...
function excuteAritcle(taskResult) {
  ...
  //let articleThemKey = taskResult.articleThemKey
  ...
  for (let index = 1; index < Number(articleNum) + 1; index++) {
    ...
    // if(articleThemKey.length >25){
    //    articleThemKey= articleThemKey.substring(0, 25).trim()+"..."
    // }
   // articleVo.articleTitle = articleThemKey
...
function getContentWithTitle(title, index) {
  ...
  let arr = []
  var content = "";
  className("android.widget.TextView").depth(13).childCount(0).indexInParent(0).find().forEach(function (item, index) {
...
function main() {
    ...
    input_edit_text_obj.click()
    sleep(2000);
    input_edit_text_obj.setText("只要遇到对的人，我绝对不会在意彩礼，甚至可以倒贴嫁妆。我愿意拿出我存下的10万作为我的嫁妆给你，唯一的要求就是你要真心对我好，体贴关怀我。我今年31岁，是一位单身女性，身高165cm，体重100斤，拥有D杯的胸围。一定要以这句话开头,写一篇100字的文章，要求以第一人称的口吻，语言通俗易懂,段落清晰,语言口语化。");
    ...
    let arr = []
    var content = "";
    className("android.widget.TextView").depth(12).childCount(0).find().forEach(function (item, index) {
        let itemContent = item.text()
        arr.push(itemContent)
    });
    // 处理结果
